...
export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("EBI_TOOLS_DATA_DO", "ebi_tools");

    server.registerTool(
        "ebi_tools_query_data",
        {
            title: "Query Staged EBI Tools Data",
            description:
                "Query staged EBI tool results using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
